  ...
  for (const bookName of bookNames) {
    try {
      await deleteWorldbookEntries(bookName, entry =>
        isManagedWorldbookEntryName(entry.name || '', rules),
      );
    } catch (e) {
...
export async function reconcileWorldbookWritesFromChat(
  ...
  await withWorldbookWriteLock(async () => {
    ...
    try {
      ...
      await deleteManagedWorldbookEntries(bookNames, rules);
...
export function registerWorldbookWriteReconcile(): EventOnReturn {
  const runInit = () => void reconcileWorldbookWritesFromChat({ reason: 'init' });
