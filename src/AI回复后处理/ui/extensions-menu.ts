  ...
  const attemptInsert = (retry: number) => {
    ...
    if (!extensionsMenu.length) {
      if (retry < MAX_RETRIES) {
        ...
        console.warn('[AI回复后处理] 魔法棒菜单 #extensionsMenu 未找到，入口注册已放弃');
      }
      return;
    }
    ...
    const $item = $(
      `<div class="list-group-item flex-container flexGap5 interactable" id="${itemId}" title="打开 ${MENU_LABEL}">` +
